...
class GoodsLogic extends Model
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
			...
			$keySpecGoodsPrice[$item_key][store_count] ? false : $keySpecGoodsPrice[$item_key][store_count] = 1000; //库存默认为0
            $str .="<td><input name='item[$item_key][price]' class='spec_price' value='{$keySpecGoodsPrice[$item_key][price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
